...
export const fetchProjectRoot = registerAction('fs:init', () => fetchPath('/').then(loadNodeData));
...
export const syncFile = registerAction('fs:sync', (params) => {
  ...
  if (!fileNode.isDir) {
    return api.readFile(path, encoding).then(loadNodeData).then((files) => files[0]);
  }
});
