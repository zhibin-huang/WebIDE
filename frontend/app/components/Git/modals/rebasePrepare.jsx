...
const TextCell = ({
  rowIndex, data, columnKey, selectedRow, ...props
}) => (
  <Cell
    {...props}
...
class GitRebasePrepare extends Component {
  ...
  getTodo() {
    const { rebaseTodoLines } = this.state;
    const todos = [];
  ...
  handleView() {
    const { commit } = this.state.rebaseTodoLines[this.state.selectedRow];
    const oldRef = `${commit}^`;
  ...
  handleDown() {
    const { rebaseTodoLines } = this.state;
    if (this.state.selectedRow == (rebaseTodoLines.length - 1)) { return; }
