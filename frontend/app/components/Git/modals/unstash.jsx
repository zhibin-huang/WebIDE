...
class GitUntashView extends Component {
  ...
  render() {
    const {
      branches, unstash, selectStash, dropStash, applyStash,
      updateUnstashIsPop, updateUnstashIsReinstate,
    } = this.props;
    const { current: currentBranch } = branches;
    const {
      stashList, selectedStash, isPop, isReinstate, newBranchName,
    } = unstash;
      ...
      <div className="stash-list">
        <ul>
          ...
          stashList.map((item, itemIdx) => (
            <li
              className={cx({ isSelected: item.rev == selectedStash.rev })}
