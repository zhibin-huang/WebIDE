...
class _GitFileTreeNode extends Component {
  ...
  render() {
    ...
    const indentCompensation = this.getIndentCompensation();
    return (
              ...
              { hideTitle ? ''
                : (
                  <span className="filetree-node-label">
                    {i18n`git.fileTree.fileStatus`}
                    { displayOnly ? (
                      <span>
                        {' '}
                        (
                        {node.leafNodes.length}
                        {' '}
                        {i18n`git.fileTree.changed`}
                        )
                        {' '}
                      </span>
                    )
                      : (
                        <span>
                          {' '}
                          (
                          {this.getStagedLeafNodes().length}
                          {' '}
                          {i18n`git.fileTree.staged`}
                          {' '}
                          /
                          {' '}
                          {node.leafNodes.length}
            ...
            : showShrinkPathView && this.shouldHideAtShrinkPath() ? null
              ...
              : (
                ...
                >
                  ...
                  <div className="filetree-node-bg" />
                </div>
              )}
  ...
  getStagedLeafNodes() {
    ...
    return node.leafNodes.filter((leafNodePath) => statusFiles.get(leafNodePath).get('isStaged'));
  }

  getChildrenStagingStatus() {
    const { node, statusFiles } = this.props;
    if (!node.isDir) return false;
    const stagedLeafNodes = this.getStagedLeafNodes();
    if (stagedLeafNodes.length == 0) {
      return 'NONE';
    } if (stagedLeafNodes.length === node.leafNodes.length) {
      return 'ALL';
    }
    return 'SOME';
  }
  ...
  shouldHideAtShrinkPath() {
    ...
    const childNode = statusFiles.get(node.children[0]);
  ...
  getIndentCompensation() {
    let {
      node, visualParentPath, indentCompensation, showShrinkPathView,
    } = this.props;
