...
import {
  showModal, addModal, dismissModal, updateModal,
} from '../Modal/actions';
...
export function showStash() {
  return (dispatch) => {
    dispatch(getCurrentBranch()).then(() => showModal('GitStash'));
  };
}
...
export function addTag({
  tagName, ref, message, force,
}) {
  return (dispatch) => api.gitAddTag({
    tagName, ref, message, force,
  }).then((res) => {
  ...
  return (dispatch) => api.gitMerge(branch).then((res) => {
    ...
    if (res.status && res.status === 'CONFLICTING') {
      ...
      api.gitStatus().then(({ files, clean }) => {
        ...
        dispatch(updateStatus({ files, isClean: clean }));
      }).then(() => showModal('GitResolveConflicts'));
    }
  }).catch((err) => {
    notify({
      notifyType: NOTIFY_TYPE.ERROR,
...
export function rebase({
  branch, upstream, interactive, preserve,
}) {
  return (dispatch) => api.gitRebase({
    branch, upstream, interactive, preserve,
  }).then((res) => {
    if (res.success) {
...
export function gitCommitDiff({ rev, title, oldRef }) {
  return (dispatch) => api.gitCommitDiff({ rev }).then((res) => {
    ...
    dispatch(updateCommitDiff({
      files, ref: rev, title, oldRef,
    }));
