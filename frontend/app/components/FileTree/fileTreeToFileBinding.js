...
export default function bindToFile(FileTreeState, FileState, FileTreeNode) {
  ...
  const dispose3 = (function shrinkPathEnhancement() {
    ...
    return reaction(() => FileTreeState.shrinkPath.enabled && [FileTreeState.entities.values(), FileTreeState.shrinkPath.directories.length],
      () => {
        ...
        // then see if we need to do something
          const _shrinkables = FileState.entities.values().filter((file) => shouldEnableShrinkPath(file) && isPathShrinkable(file));
          const _shrinkees = _shrinkables.filter((file) => !_shrinkables.includes(file.parent)).map(getShrinkeeNode);
