...
Breadcrumbs = inject((state) => {
  const { activeTab } = state.EditorTabState;
...
const Crumb = ({ node }) => {
  ...
  return (
    <div className="crumb">
      {SHOW_ICON ? <i className={node.isDir ? 'fa fa-folder-o' : (icons.getClassWithColor(node.name) || 'fa fa-file-text-o')} style={{ marginRight: '5px' }} /> : null}
