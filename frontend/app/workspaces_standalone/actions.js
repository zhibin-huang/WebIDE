...
export function fetchPublicKey() {
  return (dispatch) => {
    api.getPublicKey().then((result) => dispatch({ type: WORKSPACE_FETCH_PUBLIC_KEY, payload: result }));
  };
}
...
export function fetchWorkspaceList() {
  return (dispatch) => {
    api.getWorkspaces().then((workspaces) => dispatch({ type: WORKSPACE_FETCH_LIST, payload: workspaces }));
  };
}
...
export function deleteWorkspace(spaceKey) {
  return (dispatch) => {
    api.deleteWorkspace(spaceKey).then((res) => dispatch(fetchWorkspaceList())).catch((e) => {
      dispatch(toggleCreatingWorkspaceErr(e.response.data.msg));
    });
  };
}
